<div class="space-y-4">
    <div class="grid grid-cols-2 gap-4 text-sm">
        <div>
            <div class="text-xs font-semibold uppercase text-gray-500">Shopify Siparişi</div>
        ...
        <div>
            ...
            <div class="text-xs text-gray-500">Contact: #{{ $order->plenty_contact_id }} / Address: {{ $order->plenty_address_id ?? '—' }}</div>
        </div>
        <div>
            <div class="text-xs font-semibold uppercase text-gray-500">Durum</div>
            <div class="font-mono">{{ $order->state }} ({{ $order->attempts }} deneme)</div>
        </div>
        <div>
            <div class="text-xs font-semibold uppercase text-gray-500">Gönderim</div>
    ...
    @if($order->error)
        <div>
            <div class="text-xs font-semibold uppercase text-danger-600">Hata Mesajı</div>
    ...
    <div>
        <div class="text-xs font-semibold uppercase text-gray-500">Gönderilen Payload</div>
    ...
    @if(!empty($order->response))
        <div>
            <div class="text-xs font-semibold uppercase text-gray-500">Plenty Cevabı</div>
